...
Bubble.prototype.checkEdges = function(){
    ...
    if(this.loc.x > canvas.width)  this.x = 0; // wrap around from right to left
    if(this.loc.x < 0)  this.x = canvas.width; // wrap around from left to right
    if(this.loc.y > canvas.height)  this.y = 0; // wrap around from bottom to top
    if(this.loc.y < 0)  this.y = canvas.height; // wrap around from top to bottom
  }
